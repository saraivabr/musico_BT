...
import { vdiPlaybook } from '../src/knowledge/vdiPlaybook'
...
async function main() {
  console.log('🚀 Gerando embeddings do VDI Playbook no Google...\n')

  const embeddings: EmbeddingVector[] = []
  const total = vdiPlaybook.length

  for (let i = 0; i < vdiPlaybook.length; i++) {
    const chunk = vdiPlaybook[i]
  ...
  // Salva em arquivo
  const outputPath = path.join(process.cwd(), 'data', 'vdi-embeddings.json')
