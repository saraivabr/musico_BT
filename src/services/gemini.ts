...
function detectEmotion(message: string): string {
  ...
  if (/triste|chorando|deprimid|sozinho|desesperado|angustia/.test(lower)) return 'tristeza'
  if (/feliz|alegr|animad|gratidão|obrigad|maravilhos/.test(lower)) return 'alegria'
  if (/medo|ansios|preocupad|nervos|panico/.test(lower)) return 'ansiedade'
  if (/raiva|irritad|bravo|odio|revolta/.test(lower)) return 'raiva'
  if (/confus|perdid|não sei|dúvida/.test(lower)) return 'confusão'
