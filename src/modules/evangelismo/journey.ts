...
export function getNextStep(currentId: string): string | null {
  const idx = evangelismSteps.findIndex(s => s.id === currentId)
  if (idx === -1 || idx >= evangelismSteps.length - 1) return null
  return evangelismSteps[idx + 1].id
}

export function getStepMessage(stepId: string): string | null {
  const step = evangelismSteps.find(s => s.id === stepId)
